...
print(blue+"""
          ...
          اداة رشق مشاهدات تيكتوك
    
        مطور الااداة [ @k33331 ]    
        
        

""",green+"\n                ( قناتي التلكرام @kkkhufsd / @Y_m_eee )")
url = input('\n[?] رابط الفيديو : ')
Thread = int(input('[?] عدد المشاهدات في الثانية : '))
def get_id_vedio():
    headers = {
        'Connection': 'close',
        'Pragma': 'no-cache',
    ...
    try:
        ...
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] 😴جاري إرسال المشاهدات')
        time.sleep(2)
    except:
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] عدد المشاهدات في الثانية')
        input()
    def view():
        ...
        while True:
            ...
            if ('"status_code":0') in req_view:
                ...
                print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] هناك خطأ')
    threads = []
